...
output_file <- "output/dma_bi_statistics.csv"
...
if (length(dma_cols) == 0L || length(bi_cols) == 0L) {
    stop("未找到 DMA 或 BI 样本列。")
}
value_cols <- c(dma_cols, bi_cols)
...
result_bi <- as.matrix(result[bi_cols])
log2_dma <- log2(result_dma)
log2_bi <- log2(result_bi)
result$DMA_mean <- rowMeans(result_dma)
result$BI_mean <- rowMeans(result_bi)
result$logFC <- rowMeans(log2_dma) - rowMeans(log2_bi)
result$FC <- 2^result$logFC
calc_p <- function(i) {
        tryCatch(t.test(log2_dma[i, ], log2_bi[i, ], var.equal = TRUE)$p.value, error = function(e) NA_real_)
}
calc_d <- function(i) {
    x <- log2_dma[i, ]
    y <- log2_bi[i, ]
...
calc_ci <- function(i) {
    tryCatch(unname(t.test(log2_dma[i, ], log2_bi[i, ], var.equal = TRUE)$conf.int), error = function(e) c(NA_real_, 
        NA_real_))
}
result$pvalue <- vapply(seq_len(nrow(result)), calc_p, numeric(1))
result$Sig <- ifelse(!is.na(result$pvalue) & result$pvalue < 0.05 & result$logFC > 0.263, "Up", ifelse(!is.na(result$pvalue) & 
    result$pvalue < 0.05 & result$logFC < -0.263, "Down", ""))
...
result <- result[c("Gene_symbol", dma_cols, bi_cols, "DMA_mean", "BI_mean", "FC", "pvalue", "logFC", 
    "Sig", "adj_pvalue_BH", "Cohen_d", "log2FC_CI95_low", "log2FC_CI95_high")]
